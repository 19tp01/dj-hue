...
def pattern(
    ...
    Example:
        @pattern("s_stagger", "Random sequential flash", tags=["flash"])
        def stagger_flash() -> LightPattern:
            return light("all").seq().shuffle().color("white")
    """

    def decorator(
        fn: Callable[[], "LightPattern"],
    ) -> Callable[[], "LightPattern"]:
        meta = PatternMeta(name=name, description=description, tags=tags or [])
...
def get_registered_patterns() -> dict[str, tuple["LightPattern", str]]:
    """
    Get all registered patterns, instantiated.

    Returns:
        Dict mapping name to (LightPattern instance, description).
    """
    from .strudel import LightPattern

    result: dict[str, tuple[LightPattern, str]] = {}
    for name, (fn, meta) in _REGISTRY.items():
        try:
            pattern_instance = fn()
            result[name] = (pattern_instance, meta.description)
